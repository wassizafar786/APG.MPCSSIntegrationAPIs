...
{
    public class PaymentDebitInwardHostAdapter : IPaymentDebitInwardHostAdapter
    {
        #region Fields
        ...
        #region Constructor 
        public PaymentDebitInwardHostAdapter(IMpcssCommunicator mpcssCommunicator,
        ...
        public async Task<ServiceResponse> Execute(DirectDebitTransferResponseDto baseInternalRequest, MPCSSRecordRequest mpcssMessageType)
        {
        ...
        private MqMessage ConvertToExternalRequest(DirectDebitTransferResponseDto request, MessageRequesitesDto messageRequesites)
        {
            ...
            if (!string.IsNullOrEmpty(isSimulated) && isSimulated == "true")
            {
                ...
                ConstructSimulatedResponse(Convert.ToString(request.GrpHdr.TotalInterbankSettlementAmount), ref paymentReportResponseDto);
            }
        ...
        private void ConstructSimulatedResponse(string requestAmount, ref PaymentReportResponseDto response)
        {
            ...
            else if (requestAmount.Equals("9004"))
            {
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.GroupStatus = "RJCT";
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.StsRsnInf = new StatusInformationResponseDto()
                {
            ...
            else if (requestAmount.Equals("9005"))
            {
                //groupStatus = "RJCT";
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.GroupStatus = "RJCT";
                ...
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.StsRsnInf = new StatusInformationResponseDto()
                {
                    Reason = new ReasonInformationResponseDto()
                    {
                        OriginalGroupStatusProprietary = "1106"
                    },
                    AdditionalInformation = "Authorization Failed"
                };
            }
            else if (requestAmount.Equals("9006"))
            {
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.GroupStatus = "RJCT";
                response.FIToFIPmtStsRpt.OrgnlGrpInfAndSts.StsRsnInf = new StatusInformationResponseDto()
                {
                    Reason = new ReasonInformationResponseDto()
                    {
                        OriginalGroupStatusProprietary = "1107"
                    },
                    AdditionalInformation = "Risk threshold breached"
                };
            }  
