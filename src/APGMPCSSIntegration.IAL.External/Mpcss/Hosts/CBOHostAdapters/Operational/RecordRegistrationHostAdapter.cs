...
using APGDigitalIntegration.Common.CommonViewModels.Registeration_New.RecordManagement;
...
{
    public class RecordRegistrationHostAdapter : IRecordRegistrationHostAdapter
    {
        #region Fields
        ...
        public async Task<ServiceResponse> Execute(RegistrationRecordInputDto baseInternalRequest, MPCSSRecordRequest mpcssMessageType)
        {
            try
            {
                ...
                var internalResponse = await SendMessage(baseInternalRequest.MessageIdentifier.MessageIdentificationCode, externalRequest, mpcssMessage).ConfigureAwait(false);
        ...
        private Envelope ConvertToExternalRequest(RegistrationRecordInputDto mpcssRequest, MPCSSRecordRequest MessageType, MessageRequesitesDto messageRequesites)
        {
            Envelope externalmessage = default;

            var date = DateTime.UtcNow.ToISODateTime();
            var dateOfBirth = mpcssRequest.DobOrRegistrationDate.ToISODate();

            if (MessageType is MPCSSRecordRequest.RecordOpeningRequest)
            {
                var customerRegRequest = new CustomerRecordOpeningExternalRequest
                {
                    ...
                    CstmrInfo = new CustomerInformation
                    {
                    ...
                    Address = new AddressInformation
                    {
                        ...
                        MobNb = mpcssRequest.MobileNumber,
                ...
                customerRegRequest.CorrelationId = customerRegRequest.MsgId.Id;
                var request = new CustomerRecordOpeningExternalRequestRoot()
                {
                    CustomerRecordOpeningExternalRequest = customerRegRequest
                };
            ...
            else if (MessageType is MPCSSRecordRequest.RecordMaintenanceRequest)
            {
                var customerRegRequest = new CustomerRecordMaintenanceExternalRequest
                {
                    MsgId = new MessageIdentifier
                    {
                        Id = mpcssRequest.MessageIdentifier.MessageIdentificationCode,
                ...
                customerRegRequest.CorrelationId = customerRegRequest.MsgId.Id;
                var request = new CustomerRecordMaintenanceExternalRequestRoot()
                {
                    CustomerRecordMaintenanceExternalRequest = customerRegRequest
                };
                externalmessage = _mpcssMessageBuilder.ConvertToExternalRequest(request, date, mpcssRequest.MessageIdentifier.MessageIdentificationCode, messageRequesites.MessageType, false);
            }
            else if (MessageType is MPCSSRecordRequest.RecordClosingRequest)
            {
                var customerCloseRequest = new CustomerRecordClosingExternalRequest
                {
                    MsgId = new MessageIdentifier
                    {
                        Id = mpcssRequest.MessageIdentifier.MessageIdentificationCode,
                ...
                customerCloseRequest.CorrelationId = customerCloseRequest.MsgId.Id;
                var request = new CustomerRecordClosingExternalRequestRoot()
                {
                    CustomerRecordClosingExternalRequest = customerCloseRequest
                };
                externalmessage = _mpcssMessageBuilder.ConvertToExternalRequest(request, date, mpcssRequest.MessageIdentifier.MessageIdentificationCode, messageRequesites.MessageType, false);
            }
            return externalmessage;
        }
        private async Task<ServiceResponse> SendMessage(string msgId, Envelope message, MessageRequesitesDto messageRequesites)
        {
            ...
            {
                await _mpcssCommunicator.SendMessage(message, messageRequesites.QueueType, ActiveMQMessageTypes.Text);
            ...
            return await ConstructSimulatedResponse(msgId, messageRequesites);
        }
